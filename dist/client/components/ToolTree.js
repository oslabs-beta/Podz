...
const ToolTree = ({ setToolMetric, clusterData }) => {
    const commonKeysAndValues = (obj1, obj2) => {
        const matchingKeys = Object.keys(obj1).filter((key) => obj2.hasOwnProperty(key));
    ...
    useEffect(() => {
        ...
        const simulation = d3
            ...
            .distance((d) => {
            if (d.source.kind === 'Pod' && d.target.kind === 'Node')
                return 175;
            return 125;
        }) // link's length
        )
    ...
    return (React.createElement("div", { className: 'toolTree' },
